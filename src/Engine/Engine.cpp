...
void Engine::Start() {
	...
	GetMap().AddWall(Wall(Vector2(2.0f, -4.4f), Vector2(0.0f, 3.0f), Color(1.0f, 0.0f, 0.5f)));
	GetMap().AddWall(Wall(Vector2(-1.0f, -3.4f), Vector2(-2.0f, 0.0f), Color(1.0f, 0.5f, 0.0f)));
	GetMap().AddWall(Wall(Vector2(4.0f, -8.7f), Vector2(3.0f, 5.0f), Color(0.0f, 1.0f, 0.0f)));
...
Color& Engine::GetSkyColor() {
	return _skyColor;
}

Color& Engine::GetFloorColor() {
	return _floorColor;
}
